...
$('#analysis-button').click(function () {
    ...
    let urlWeb = $('#url').val().replace(match, "");
    let title = '';
    let button = '';
    let htmlFill = '';
    if (lang === 'en') {
...
function listener() {
    for (let i = 0; i < 5; i++) {
        $('#nav-' + categories[i]).click(function () {
            hideResult();
...
function hideResult() {
    for (let j = 0; j < 5; j++) {
        $('#' + categories[j]).css('display', 'none');
    }
}

function strokeValue(score, number, category) {
    let card = $('.' + category);
    // let ribbon = $('#ribbon-'+category);
    let value = $('.value-' + category);
...
function addSubTitle(title, id, description = null, category) {
    if (description == null) {
        $('.' + category + '-audit').append("<h3 style=\"color:black\">" + converter.makeHtml(title) + "</h3>\n" +
            "    <div id=\"" + id + "\"></div><br>")
    } else {
        $('.' + category + '-audit').append("<br><h3 style=\"color:black\">" + converter.makeHtml(title) + "</h3>\n" +
...
function addItem(allAudits, audit, category, group = null) {
    ...
    // $('.' + category + '-audit #' + groupId).append("<hr>\n" +
    ...
    //     table +
    //     "    </div>\n");
    $('.' + category + '-audit #' + groupId).append("<div class=\"card\">\n" +
...
function refreshAuditsResult() {
    for (let j = 0; j < 5; j++) {
        $('#' + categories[j] + '-audit').empty();
        $('#pass-' + categories[j] + '-audit').empty();
        $('#not-app-' + categories[j] + '-audit').empty();
        $('#manual-' + categories[j] + '-audit').empty();
    }
}
